...
OTHER_APP_ANDROID = 'http://mobilestore.douban.com/app/android/fm',
OTHER_APP_IOS = 'http://mobilestore.douban.com/app/ios/fm',
API = 'http://music.douban.com/api/artist',
SAPI = 'https://music.douban.com/api/artist',
ANDROID_APP_VERSION ='1.0.9',
        ...
        return function(r) {
            if (r['r'] == 1 && r['err']) {
                if (r.err == 'need_token' ||
                    r.err == 'invalid_token' ||
                    r.err == 'expired') {
                    logout_user(do_confirm.OK);
                } else {
                    do_alert(err_msg(r.err), '错误');
                }
                return;
            }
            var cb = $.getpcb[key];
...
$(function(){
    ...
    */
    ...
    $('#next_btn').click(function(){
        player.play_next(true); //record skip action
    });
